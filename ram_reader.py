...
class PokemonGoldRAM:
    ...
    PARTY_MON1_PP_START   = 0xDA41   # PP der 4 Moves (Offset +0x17); untere 6 Bits = PP
    ...
    JOHTO_BADGES         = 0xD57C   # ✓ Gold/Silber Johto-Orden, EMPIRISCH bestätigt (Bit0=Falkner).
                                    #   War 0xD857 = Crystal-Adresse → für Gold falsch (blieb immer 0).
...
def read_pp(pyboy: PyBoy) -> int:
    """
    Summe der aktuellen PP von Mon 1 über alle 4 Move-Slots.

    Untere 6 Bits = aktuelle PP, obere 2 Bits = PP-Ups (ausmaskiert via & 0x3F).
    Adresse 0xDA41–0xDA44 (Offset +0x17 im Party-Struct ab 0xDA2A).

    HINWEIS: Gesamt-PP als "kann nicht angreifen"-Signal ist trügerisch –
    Status-Moves (z.B. Leer) behalten ihre PP, daher fällt die Summe nie auf 0.
    Für den PP-AUFFÜLL-Reward (Center heilt alle Moves) ist die Summe aber korrekt.
    Verifiziere mit debug_ram.py: im Kampf angreifen → Wert sinkt.
    """
    if read_party_count(pyboy) == 0:
        return 0
    total = 0
    for i in range(4):
        total += pyboy.memory[PokemonGoldRAM.PARTY_MON1_PP_START + i] & 0x3F
    return total
...
def read_party_total_hp(pyboy: PyBoy) -> int:
    """
    Summe der aktuellen HP ALLER Team-Pokemon. 0 = Blackout (alle ohnmächtig).

    Party-Mon-Struct: 0x30 (48) Bytes ab 0xDA2A, aktuelle HP bei Offset +0x22
    (High) / +0x23 (Low), Big-Endian – gleiche Struktur wie read_total_party_xp.
    Wird genutzt, um Tod-Auto-Heal (Blackout) von freiwilligem Pokécenter-Heal
    zu unterscheiden.
    """
    total = 0
    for i in range(read_party_count(pyboy)):
        base = 0xDA2A + i * 0x30
        total += (pyboy.memory[base + 0x22] << 8) | pyboy.memory[base + 0x23]
    return total


def read_party_total_level(pyboy: PyBoy) -> int:
    """
    Summe der Level ALLER Team-Pokemon. Steigt bei jedem Level-Up – UND beim Fangen
    (neues Mon bringt sein Level ein); der Aufrufer schließt Fänge über party_count
    aus, sonst Doppelzählung mit dem Fang-Bonus.
    Level-Offset +0x1F im 0x30-Struct ab 0xDA2A (= 0xDA49 für Mon 1).
    """
    total = 0
    for i in range(read_party_count(pyboy)):
        total += pyboy.memory[0xDA2A + i * 0x30 + 0x1F]
    return total
...
def read_has_egg(pyboy: PyBoy) -> int:
    """
    1 = die KI hat die EI-LINIE im Team (Ei ODER geschlüpftes Togepi), sonst 0.

    Ein Ei belegt einen Team-Slot mit Spezies-Marker 0xFD (253, "EGG") in der
    Party-Spezies-Liste (ab 0xDA23); nach dem SCHLÜPFEN wird daraus Togepi (175).
    Wir zählen BEIDES → der Wert bleibt nach dem Schlüpfen auf 1, statt auf 0 zu
    fallen (Metrik/Observation/Reward unterzählen das Abholen nicht mehr).
    ✓ 175=Togepi empirisch bestätigt (beim Ei-Abholen tauchte 175 kurz auf, bevor
    0xFD gesetzt wurde). Spiel-Gate: erst MIT der Ei-Linie kommt man von Violet
    nach Route 32 Süd.
    """
    species = read_party_species(pyboy)
    return int(0xFD in species or 175 in species)
...
def read_badge_count(pyboy: PyBoy) -> int:
    """
    Zählt die Anzahl der Johto-Abzeichen.

    Johto-Abzeichen: Bitfeld in Byte 0xD57C (✓ empirisch bestätigt).
      Bit0 ($01)=Falkner, $02=Bugsy, $04=Whitney, $08=Morty,
      $10=Jasmine, $20=Chuck, $40=Pryce, $80=Clair.
...
def read_battle_type(pyboy: PyBoy) -> int:
    """
    Roher Kampftyp aus 0xD116:  0 = kein Kampf, 1 = Wild, 2 = Trainer, höher = Spezial.
    ✓ Verifiziert: Log zeigt "Train" bei Trainer- und "Wild" bei Wildkämpfen.
    """
    return pyboy.memory[PokemonGoldRAM.BATTLE_TYPE]
...
def get_all_ram_features(pyboy: PyBoy, coord_transform=None) -> np.ndarray:
    ...
    pp                          = read_pp(pyboy)
    total_level                 = read_party_total_level(pyboy)
    has_egg                     = read_has_egg(pyboy)
    ...
    features = np.array([
        ...
        min(total_level, 600) / 600.0,   # Team-Gesamtlevel [0.0, 1.0] (6×Lvl100=600)
        min(pp, 200) / 200.0,            # aktuelle PP Mon1, alle Moves [0.0, 1.0]

        # --- Fortschritt ---
        caught_count / 251.0,        # Gefangene Pokemon [0.0, 1.0]
        badge_count / 16.0,          # Abzeichen [0.0, 1.0] (16 total)
        float(has_egg),              # 1.0 = Ei im Team (Gate: Violet → Route 32 Süd)
