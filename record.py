...
# VIDEO_FPS = 60 → Video läuft in echter GameBoy-Geschwindigkeit (wie ein YouTube-Playthrough)
VIDEO_FPS    = 60
...
# Savestatepath="./Savestates/PGV_Route30_one_cought_mid.state"
Savestatepath=SAVE_STATE_PATHS[3]
...
def load_model() -> PPO:
    ...
    """
    candidates = [
        "./checkpoints/pokemon_ppo_2500000_steps.zip",
    ...
    try:
        while step < MAX_RECORD_STEPS:
            ...
            if step % 200 == 0:
                ...
                print(f"    Schritt {step}/{MAX_RECORD_STEPS}  |  Frames: {frame_count}  |  Reward: {total_r:.2f}  |  Karten: {maps}")
...
def _add_info_overlay(frame: np.ndarray, info: dict, step: int, total_reward: float) -> np.ndarray:
    ...
    line1 = f"Schr.: {step:4d}   Reward: {total_reward:+7.2f}   Maps: {maps}"
    line2 = f"Felder: {tiles:4d}   Lv: {level}   Gef: {caught}   HP: {hp_r*100:.0f}%"
...
def record():
    ...
    for ep in range(1, N_RECORD_EPISODES + 1):
        ...
        tmp_path = os.path.join(VIDEO_PATH, f"_recording_ep{ep:03d}_tmp.mp4")
        ...
        final_path = os.path.join(VIDEO_PATH, filename)
        os.rename(tmp_path, final_path)
        print(f"  ✓ Video gespeichert: {final_path}")

    print(f"\n✓ Alle Videos gespeichert in: {VIDEO_PATH}")
